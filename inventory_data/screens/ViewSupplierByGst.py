...
class ViewSupplierByGst(QWidget):
    def __init__(self):
        super().__init__()
        self.PrepareScreen()

    def PrepareScreen(self):
        self.setWindowTitle("View Supplier BY GST No Screen")
        self.setGeometry(300, 100, 500, 400)
        grid = QGridLayout()
        grid.setSpacing(10)
        lblsuppliergst=QLabel("Enter Supplier GST No")
        self.suppliergstEdit=QLineEdit()
        self.btn=QPushButton("View")
        self.tablewidget=QTableWidget()
        self.tablewidget.setStyleSheet("background-color:#CCDADA;")
        newfont = QFont("Bell MT", 18, QFont.Bold)
        lblsuppliergst.setFont(newfont)
        self.suppliergstEdit.setFont(newfont)
        self.btn.setFont(newfont)

        grid.addWidget(lblsuppliergst,1,0,1,2)
        grid.addWidget(self.suppliergstEdit, 2, 0, 1, 2)
        ...
        try:
            ...
            if IsEmpty(gst):
                ...
                if gst is not None:
                    ...
                    query = "select * from supplierinfo where GSTno="+str(gst)
                    print(query)
                    row=0
