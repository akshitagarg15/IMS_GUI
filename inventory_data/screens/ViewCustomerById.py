...
class ViewCustomerById(QWidget):
    def __init__(self):
        super().__init__()
        self.PrepareScreen()

    def PrepareScreen(self):
        self.setWindowTitle("View Customer BY Id Screen")
        self.setGeometry(300, 100, 500, 400)
        grid = QGridLayout()
        grid.setSpacing(10)
        lblcustid=QLabel("Enter Customer Id")
        self.custidEdit=QLineEdit()
        ...
        self.custidEdit.setFont(newfont)
        self.btn.setFont(newfont)

        grid.addWidget(lblcustid,1,0,1,2)
        grid.addWidget(self.custidEdit, 2, 0, 1, 2)
        ...
        try:
            ...
            if IsEmpty(id):
                ...
                if id is not None:
                    ...
                    query = "select * from customerinfo where CustomerId="+str(id)
                    print(query)
                    row=0
